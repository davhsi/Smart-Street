                ...
                <div className="space-y-6">
                  ...
                  <div className={`p-4 rounded-xl border ${request.status === 'APPROVED' ? 'bg-green-50 dark:bg-green-900/20 border-green-200 dark:border-green-800' :
                    ...
                    <div className="flex justify-between items-center">
                      ...
                      <span className={`px-3 py-1 rounded-md text-sm font-bold uppercase ${request.status === 'APPROVED' ? 'bg-green-100 text-green-700 dark:bg-green-900 dark:text-green-300' :
                    ...
                    {request.remarks && (
                      <div className={`mt-3 pt-3 border-t ${request.status === 'REJECTED' ? 'border-red-200 dark:border-red-800/50' :
                          request.status === 'APPROVED' ? 'border-green-200 dark:border-green-800/50' :
                            'border-slate-200 dark:border-slate-700'
                        }`}>
                        <p className={`text-sm font-bold mb-1 ${request.status === 'REJECTED' ? 'text-red-600 dark:text-red-400' :
                            request.status === 'APPROVED' ? 'text-green-600 dark:text-green-400' :
                              'text-slate-600 dark:text-slate-400'
                          }`}>Admin Remarks:</p>
                        <p className={`text-base ${request.status === 'REJECTED' ? 'text-red-700 dark:text-red-300' :
                  ...
                  <div className="grid grid-cols-2 gap-4 text-base">
                    <div>
                      ...
                      <p className="font-mono text-slate-900 dark:text-white text-base">#{request.request_id}</p>
                    </div>
                    <div>
