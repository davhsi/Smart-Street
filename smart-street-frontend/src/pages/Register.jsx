      ...
      <div className="relative w-[90%] md:w-full max-w-3xl bg-white/60 dark:bg-white/5 backdrop-blur-xl shadow-2xl rounded-2xl p-6 md:p-8 border border-white/40 dark:border-white/10 z-10 transition-all">
        <div className="text-center mb-8">
          <Link to="/" className="inline-block hover:opacity-80 transition-opacity">
            <h2 className="text-sm font-semibold text-cyan-600 dark:text-cyan-500 tracking-[0.3em] uppercase mb-2">{t("join_smart_street")}</h2>
          </Link>
          <h1 className="text-3xl font-bold text-slate-800 dark:text-white tracking-tight">{t("create_your_account")}</h1>
        </div>
          ...
          <div className="flex bg-slate-100 dark:bg-slate-900/80 p-1.5 rounded-xl border border-slate-200 dark:border-slate-700/50">
            {[
              ...
              <button
                ...
                className={`flex items-center gap-2 px-6 py-2.5 rounded-lg text-sm font-semibold transition-all duration-300 ${activeRole === role.id
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            {/* Common Fields */}
            <InputField label={t("full_name")} icon={UserIcon} value={form.name} onChange={e => updateField("name", e.target.value)} placeholder="John Doe" />
            <InputField label={t("phone_number")} icon={PhoneIcon} value={form.phone} onChange={e => updateField("phone", e.target.value)} placeholder="+1 234 567 890" type="tel" />
            <InputField label={t("email_address")} icon={EnvelopeIcon} value={form.email} onChange={e => updateField("email", e.target.value)} placeholder="john@example.com" type="email" />
            <InputField label={t("password")} icon={LockClosedIcon} value={form.password} onChange={e => updateField("password", e.target.value)} placeholder="••••••••" type="password" minLength={8} />
          </div>
          ...
          <div className="pt-4 border-t border-slate-200 dark:border-white/10">
            <h3 className="text-xs font-bold text-slate-500 dark:text-slate-300 mb-4 uppercase flex items-center gap-2">
              {activeRole === "VENDOR" && <><BuildingStorefrontIcon className="w-4 h-4" /> {t("vendor_details")}</>}
              {activeRole === "OWNER" && <><KeyIcon className="w-4 h-4" /> {t("owner_details")}</>}
              {activeRole === "ADMIN" && <><LockClosedIcon className="w-4 h-4" /> {t("security_clearance")}</>}
            </h3>

            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
              {activeRole === "VENDOR" && (
                <>
                  <InputField label={t("business_name")} icon={BriefcaseIcon} value={form.businessName} onChange={e => updateField("businessName", e.target.value)} placeholder="John's Food Truck" />
                  <InputField label={t("category")} icon={BriefcaseIcon} value={form.category} onChange={e => updateField("category", e.target.value)} placeholder="Food & Beverage" />
                  <InputField label={t("license_number")} icon={IdentificationIcon} value={form.licenseNumber} onChange={e => updateField("licenseNumber", e.target.value)} placeholder="LIC-12345678" className="md:col-span-2" />
                </>
              )}

              {activeRole === "OWNER" && (
                <>
                  <InputField label={t("owner_entity_name")} icon={UserIcon} value={form.ownerName} onChange={e => updateField("ownerName", e.target.value)} placeholder="City Council / Pvt Ltd" />
                  <InputField label={t("contact_info_public")} icon={PhoneIcon} value={form.contactInfo} onChange={e => updateField("contactInfo", e.target.value)} placeholder="Public helpline or email" />
                </>
              )}

              {activeRole === "ADMIN" && (
                <div className="md:col-span-2">
                  <InputField label={t("admin_access_code")} icon={KeyIcon} value={form.adminCode} onChange={e => updateField("adminCode", e.target.value)} placeholder="Enter secure registration code" />
                  <p className="text-[10px] text-slate-500 mt-2 italic">{t("admin_code_note")}</p>
                </div>
              )}
            </div>
          </div>
          ...
          <div className="pt-2 flex flex-col md:flex-row items-center justify-between gap-4">
            <Link className="text-sm text-slate-500 dark:text-slate-400 hover:text-slate-800 dark:hover:text-white transition-colors flex items-center gap-1 group" to="/login">
              <span className="group-hover:-translate-x-1 transition-transform">←</span> {t("back_to_signin")}
            </Link>
            ...
            >
              {loading ? t("creating_profile") : t("complete_registration")}
            </button>
          </div>
        </form>
      </div>
